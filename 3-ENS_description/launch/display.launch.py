...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='ENS_description').find('ENS_description')
    default_model_path = os.path.join(pkg_share, 'urdf/ENS.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/ENS/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'ENS', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
