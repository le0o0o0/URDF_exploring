...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='industrial_description').find('industrial_description')
    default_model_path = os.path.join(pkg_share, 'urdf/industrial.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/industrial/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'industrial', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
